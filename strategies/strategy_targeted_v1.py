"""Targeted On-Device Strategy v1: Fix specific 0-score failure patterns.

Copies the proven generalized on-device logic from main.py, then applies
five surgical fixes to the argument extraction and tool selection layers:

1. Minute extraction: preserve H:MM patterns like 8:15, 7:30, 6:45
2. Tool selection: boost create_reminder when "remind" keyword present
3. search_contacts.query: enforce string type, extract name
4. send_message.message: clamp to extracted phrase, not model boilerplate
5. set_timer.minutes: flatten nested dicts and enforce int
"""
...
# ============ Rule-based argument extraction (FIXED) ============
...
def _extract_time_hours_minutes(text):
    """FIX #1: Preserve H:MM patterns like 8:15, 7:30, 6:45."""
    # "H:MM AM/PM"
...
def _extract_recipient_and_message(text):
    """FIX #4: Clamp message to extracted 'saying ...' phrase only."""
    # "to <Name> saying <message>"
    m = re.search(r'(?:to|tell)\s+(\w+)\s+(?:saying|that|:)\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
    ...
    m = re.search(r'(\w+)\s+a\s+message\s+saying\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
        name = m.group(1).strip()
    ...
    m = re.search(r'(?:text|message|msg)\s+(\w+)\s+(?:saying|that|:)\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
    ...
    m = re.search(r'(?:text|message)\s+(\w+)\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
        name = m.group(1).strip()
    ...
    # "to <Name> saying <msg>"  (broader)
    m = re.search(r'to\s+(\w+)\s+saying\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
        return m.group(1).strip(), m.group(2).strip().rstrip('.')

    # "tell <Name> <message>"
    m = re.search(r'tell\s+(\w+)\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
        return m.group(1).strip(), m.group(2).strip().rstrip('.')
...
def _extract_song(text):
    m = re.search(r'play\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
        ...
        if len(words) == 2 and words[-1].lower() == "music":
            ...
            if words[0].lower() not in adjective_genres:
                song = words[0]
        return song
    m = re.search(r'listen\s+to\s+(.+?)(?:\.|,\s+|$)', text, re.IGNORECASE)
    if m:
        return m.group(1).strip().rstrip('.')
    return None
...
def _extract_contact_query(text):
    """FIX #3: Strict name extraction for search_contacts.query."""
    # "find/look up/search for <Name>"
...
def _extract_args_for_tool(tool_name, user_text, cactus_args):
    if tool_name == "get_weather":
        ...
        if isinstance(raw_min, dict):
            # Nested dict like {"minutes": {"minutes": -15}}
...
def _select_tool_by_keywords(user_text, tools):
    """FIX #2: Boost create_reminder when 'remind' is present."""
    text_lower = user_text.lower()
    available_names = {t.get("name") for t in tools}

    # Priority boost: if "remind" appears, prefer create_reminder
    if "remind" in text_lower and "create_reminder" in available_names:
...
def _call_cactus_single(user_text, all_tools):
    ...
    for call in calls:
        ...
        if not isinstance(args, dict):
            args = {}
        # ALWAYS re-extract args using rules (with cactus args as fallback)
