"""On-device + cloud fallback strategy v1.

Base: on-device v3.1 (tool pruning + rule-based argument extraction).
Upgrade: keep on-device as primary path, then fallback to Gemini cloud
when local output is structurally risky.
"""
...
def _generate_local(messages, tools):
    """On-device generation with tool pruning and rule-based argument extraction."""
    ...
    return _process_single(user_text, tools)
...
def _should_fallback(local, messages, tools):
    ...
    # If tools exist, empty output is always wrong.
    ...
    for call in local_calls:
        ...
        if isinstance(minutes, int) and minutes < 0:
            return True, "negative_minutes"

    user_text = _messages_to_user_text(messages)
    # Multi-action under-calls are high-risk.
    if _is_multi_action(user_text, tools) and len(local_calls) < 2:
...
def _generate_cloud(messages, tools, model_name):
    ...
    if not contents:
        contents = [str(m.get("content", "")) for m in messages if "content" in m]
...
def generate_hybrid(messages, tools, confidence_threshold=0.99):
    """On-device first, cloud fallback on structural-risk cases."""
    local = _generate_local(messages, tools)
    ...
    # Stop retry storms if cloud is consistently unreachable.
    if getattr(generate_hybrid, "_cloud_failure_count", 0) >= 2:
        local["source"] = "on-device"
        local["fallback_reason"] = f"{fallback_reason}|cloud_temporarily_disabled"
        return local
    ...
    for model_name in (
        ...
        os.environ.get("GEMINI_MODEL_FALLBACK_3", ""),
    ):
    ...
    for model_name in models_to_try:
        try:
            ...
            if not cloud.get("function_calls"):
                last_error = f"{model_name}: empty_function_calls"
                continue
            generate_hybrid._cloud_failure_count = 0
    ...
    generate_hybrid._cloud_failure_count = getattr(generate_hybrid, "_cloud_failure_count", 0) + 1
    local["source"] = "on-device"
    local["fallback_reason"] = fallback_reason
    if last_error:
        local["cloud_error"] = last_error
    return local
